    ...
    return <nav className={cn("fixed w-full z-40 transition-all duration-300", isScrolled ? "py-3 bg-background/80 backdrop-blur-md shadow-xs" : "py-5")}>
        ...
        <div className='container flex items-center justify-between' >
            <a className='text-xl font-bold text-primary flex items-center' href='#hero'>
                <span className='relative translate-z-10'>
                    {" "}
                    <span className='text-glow text-foreground'> Mayank Kumar</span>{" "} Portfolio
                </span>
            </a>
            ...
            <div className='hidden md:flex space-x-8'>
                {navItems.map((item, key) => (
                    <a key={key} href={item.href} className=" text-foreground/80 mx-3 hover:text-primary transition-colors duration-300">
                        {item.name}
                    </a>
                ))}
            </div>
            ...
            <div className={cn("fixed inset-0 bg-background/95 backdrop-blur-md z-40 flex flex-col items-center justify-center" , "transition-all duration-300 md:hidden",
                ...
                <div className='flex flex-col space-y-8 text-xl'>
                    {navItems.map((item, key) => (
                        <a key={key} href={item.href} className=" text-foreground/80 mx-3 hover:text-primary transition-colors duration-300" onClick={()=> setIsMenuOPen(false)}>
                            {item.name}
                        </a>
                    ))}
                </div>
